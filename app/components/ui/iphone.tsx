import { forwardRef, useImperativeHandle, useRef, useEffect, type HTMLAttributes } from "react"
...
export const Iphone = forwardRef<IphoneHandle, IphoneProps>(({
  src,
  videoSrc,
  startTime,
  endTime,
  ...
  useEffect(() => {
    ...
    const handleLoadedMetadata = () => {
      // For pre-cropped videos, start from beginning
      // For legacy usage with startTime, seek to that position
    ...
    const handleTimeUpdate = () => {
      // Legacy: handle time range looping if startTime/endTime provided
      if (startTime !== undefined && endTime !== undefined && video.currentTime >= endTime) {
        video.currentTime = startTime
        if (loop) safePlay()
      }
    }
  ...
  }, [startTime, endTime, videoSrc, loop])
  ...
  return (
    ...
    >
      ...
      {!hasMedia && (
        <div
          className="absolute z-0 overflow-hidden bg-black flex items-center justify-center"
          style={{
            left: `${LEFT_PCT}%`,
            top: `${TOP_PCT}%`,
            width: `${WIDTH_PCT}%`,
